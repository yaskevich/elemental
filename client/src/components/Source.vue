        ...
        <n-form ref="formRef" :label-width="80" :model="form" :rules="rules">
            <n-space vertical>
                <n-tag
                    type="success"
                    size="large"
                    v-if="cjsObject?.data?.length === 1"
                >{{ cjsObject.data[0].id }}</n-tag>
                <n-alert
                    title="Batch mode"
                    type="warning"
                    v-if="cjsObject?.data?.length > 1"
                >You have put the dataset containing more than one BibTex record. The data will be processed in batch mode: the same language will be set for every record. After processing will be completed, you will be forwarded to the list with all sources.</n-alert>
...
const cjsObject = computed(() => { try { return new Cite(form.bib, { generateGraph: false }) } catch (error) { return new Cite('') } });
...
onBeforeMount(async () => {
    if (form.id) {
        const data = await store.get('source', form.id);
